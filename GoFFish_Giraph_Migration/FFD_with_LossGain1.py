...
def pack(values, maxValue):
    ...
    ########### LPT implementation ########################
def schedule(values, number_of_bins):
    values = sorted(values, reverse=True)
    #print str(values)
    bins = []
        ...
        if(flag==1):
            ...
            for bin in bins:
                ...
                for item in bin.items:
                    ...
                    for p in partTime2.keys():
                        if int(partTime2[p])==item:
                            key_to_remove=p
                            l.append(p)
                            break

                    partTime2.pop(key_to_remove)
            ...
            # exit()
            print len(SStimeList)
            print len(set(SStimeList))
            ...
            if (migration_cost + maxTime <= UPPER_LIMIT): ### FFD worked
            ...
            ####update the partition->vm map and vm->partition map
            ...
            else: ########### FFD violates the constraints... using loss gain avoid migration
                ...
                if(send_bottleneck_flag):
                    ...
                    if(sender_vm==-1):
                        ...
                        print "sender vm does not exist line 720"
                        exit()
                ...
                migration_count=max_migration_partition_count(VM_partition_send_map, VM_partition_receive_map)
                ...
                if((max_migration_time+max_compute_time) <= UPPER_LIMIT): ###loss gain approach worked with avoid of migration

                    print "**********************constraint satisfied after LossGain**********************************************"

                    #TODO: update the metrics and proceed to next superstep
                    #FIXME: handle the exceptional case of max score partition
                    for bin in logical_bin.keys():
                        ...
                        if(send_bottleneck_flag):
                            #map the pid to the bottleneck vn

                            l_new=PhysicalVM_Partition_Map[bottleneck_vmid]
                            l_new.append(max_score_pid)
                            PhysicalVM_Partition_Map[bottleneck_vmid]=l_new

                            Partition_PhysicalVM_Map[max_score_pid]=bottleneck_vmid

                        else:

                            #map the vm to the send_vm
                            l_new = PhysicalVM_Partition_Map[sender_vm]
                            l_new.append(max_score_pid)
                            PhysicalVM_Partition_Map[sender_vm] = l_new

                            Partition_PhysicalVM_Map[max_score_pid] = sender_vm


                    superstep+=1

                else: ############constraint not satisfied after LossGain trying for VM++

                    label .end

                    print "**********************constraint not satisfied after LossGain trying for VM++ **********************************************"
                    #TODO: VM++
                    #TODO: recompute the bins and send receive maps
                    # logical_bin.clear()
                    number_of_bins_to_use=len(logical_bin_to_PhysicalVM_mapping.keys())+1

                    logical_bin.clear()
                    vm_ActivePartition_time.clear()
                    VM_partition_receive_map.clear()
                    VM_partition_send_map.clear()
                    logical_bin_to_PhysicalVM_mapping.clear()

                    next_VMID_to_use=next_VMID_to_use_at_ss_start

                    ######### calling load balancing Algo() ##########
                    bins=schedule(SStimeList, number_of_bins_to_use)

                    for bin in bins:

                        print " bin has sum "+str(sum(bin.items))
                    ...
                    for bin in logical_bin.keys():
                        ...
                        for partition in logical_bin[bin]:
                            ...
                            if (partition in available_partition_on_vm):
                                ...
                                if (Partition_PhysicalVM_Map[
                                        partition] != -1):  # FIXME: this ensure that partitions are preloaded for first use
                                    VM_partition_receive_map[mapped_vm].add(partition)
                                    VM_partition_send_map[Partition_PhysicalVM_Map[partition]].add(partition)
                                else:
                                    print "preload partition " + str(partition) + " on vm " + str(mapped_vm)
                    ...
                    if (migration_cost + maxTime <= UPPER_LIMIT):
                    ...
                    else: ######################### "constraint not satisfied by VM++ going with placement of prev superstep"

                        print "constraint not satisfied by VM++"

                        #continue with same mapping as the previous superstep
                        ...
                        for p in partTime.keys():
                            ...
                            if(Partition_PhysicalVM_Map[p]!=-1):

                                allocated_vm=Partition_PhysicalVM_Map[p]

                                if( allocated_vm in  vm_ActivePartition_time.keys() ):

                                    vm_ActivePartition_time[allocated_vm]+=partTime[p]

                                else:
                                    vm_ActivePartition_time[allocated_vm]=partTime[p]

                            else:
                                unplaced_partitions.append(p)


                        #TODO: get the least loaded vm and place the unplaced partitions

                        for p in unplaced_partitions:

                            least_loaded_vm=min(vm_ActivePartition_time, key=vm_ActivePartition_time.get)

                            Partition_PhysicalVM_Map[p]=least_loaded_vm

                            print "preload partition "+str(p)+" on vm "+str(least_loaded_vm)

                            p_list=PhysicalVM_Partition_Map[least_loaded_vm]
                            p_list.appned(p)
                            PhysicalVM_Partition_Map[least_loaded_vm]=p_list

                            vm_ActivePartition_time[least_loaded_vm]+=partTime[p]
                        ...
                        #go with number of vm as in the previous step
